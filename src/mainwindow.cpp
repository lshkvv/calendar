...
void MainWindow::setupViews()
{
...

    {
        ...
        auto *td1 = new TagChipDelegate(ui->wishlistTableView);
        auto *td2 = new TagChipDelegate(ui->calendarTableView);
        td1->setTagColors(tagColors);
        td2->setTagColors(tagColors);
        ui->wishlistTableView->setItemDelegateForColumn(4, td1);
        ui->calendarTableView->setItemDelegateForColumn(4, td2);
    }
...
void MainWindow::loadTodayTab()
{
    ...
    while (q.next()) {
        ...
        if (d == today)                 section = "Сегодня";
        else if (d == today.addDays(1)) section = "Завтра";
        ...
        m_todayModel->appendRow({sectionItem, dateItem, timeItem, titleItem, catItem});
    }
}
...
void MainWindow::loadWishlistTab()
{
    ...
    while (q.next()) {
        ...
        m_wishlistModel->appendRow({
            new QStandardItem(d.isValid() ? d.toString("dd.MM.yyyy") : q.value("date").toString()),
            new QStandardItem(t.isValid() ? t.toString("HH:mm")      : q.value("time").toString()),
            new QStandardItem(q.value("title").toString()),
            new QStandardItem(q.value("category_name").toString()),
            new QStandardItem(m_db->getTagsStringForEvent(eventId))
        });
    }
}
...
void MainWindow::loadCalendarTab(const QDate &date)
{
    ...
    while (q.next()) {
        ...
        m_calendarModel->appendRow({dateItem, timeItem, titleItem, catItem, tagsItem});
    }
}
...
void MainWindow::onAddCategory()
{
    ...
    if (!m_db->addCategory(name, dlg.color().name()))
        QMessageBox::warning(this, "Ошибка", "Не удалось добавить категорию. Такое название уже существует.");
...
void MainWindow::onEditCategory()
{
    ...
    if (rows.isEmpty()) { QMessageBox::information(this, "Выбор", "Выберите категорию."); return; }
...
void MainWindow::onDeleteCategory()
{
    ...
    if (QMessageBox::question(this, "Удаление",
                              QString("Удалить «%1»?").arg(name)) != QMessageBox::Yes) return;
...
{
    auto rows = ui->eventsTableView->selectionModel()->selectedRows();
    if (rows.isEmpty()) { QMessageBox::information(this, "Выбор", "Выберите событие."); return; }
...
void MainWindow::onDeleteEvent()
{
    ...
    if (QMessageBox::question(this, "Удаление",
                              QString("Удалить «%1»?").arg(title)) != QMessageBox::Yes) return;
...
void MainWindow::onAddTag()
{
    ...
    if (!m_db->addTag(name.trimmed()))
        QMessageBox::warning(this, "Ошибка", "Не удалось добавить тег. Такой тег уже существует.");
...
void MainWindow::onEditTag()
{
    ...
    if (rows.isEmpty()) { QMessageBox::information(this, "Выбор", "Выберите тег."); return; }
...
void MainWindow::onDeleteTag()
{
    ...
    if (QMessageBox::question(this, "Удаление", msg) != QMessageBox::Yes) return;
